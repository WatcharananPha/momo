...
class Settings(BaseSettings):
    PROJECT_NAME: str = "MaidBooking Loyalty API"
    API_V1_STR: str = "/api/v1"
    PORT: int = int(os.getenv("PORT", 9090))
    DATABASE_URL: str = os.getenv("DATABASE_URL", "")
    JWT_SECRET: str = os.getenv("JWT_SECRET", "")
    JWT_EXPIRES_IN: str = os.getenv(
        "JWT_EXPIRES_IN", "86400"
    )  # Changed to str to handle values like '7d' from .env
    ...
    LINE_NOTIFY_MOCK_MODE: bool = (
        os.getenv("LINE_NOTIFY_MOCK_MODE", "false").lower() == "true"
    )
